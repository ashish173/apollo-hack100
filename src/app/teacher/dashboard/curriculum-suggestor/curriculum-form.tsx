...
import { generateCurriculumSuggestionsDirectAPI, parseCurriculumSuggestions, type CurriculumSuggestion, type GenerateCurriculumSuggestionsInput } from '@/lib/gemini-curriculum-api';

// Use the types from the API library
type LessonSuggestion = CurriculumSuggestion;

export default function CurriculumSuggestor() {
  ...
  const handleGenerateSuggestions = async () => {
    ...
    try {
      const input: GenerateCurriculumSuggestionsInput = {
      ...
      const result = await generateCurriculumSuggestionsDirectAPI(input);
      const parsedSuggestion = parseCurriculumSuggestions(result.curriculumSuggestions);
      setGeneratedSuggestion(parsedSuggestion);
    } catch (error) {
      console.error('Error generating curriculum suggestions:', error);
      // You might want to show an error toast here
      // toast({
      //   title: "Generation Failed",
      //   description: "Could not generate curriculum suggestions. Please try again.",
      //   variant: "destructive",
      // });
    } finally {
  ...
  return (
    <div className="flex-grow flex flex-col p-6 space-y-8 mx-auto max-w-4xl">
