...
const GenerateProjectIdeasInputSchema = z.object({
  topic: z.string().describe('The topic or subject to generate project ideas for.'),
...
const GenerateProjectIdeasOutputSchema = z.object({
  ideas: z.array(z.string()).describe('An array of project ideas.'),
});
...
const prompt = ai.definePrompt({
  ...
  prompt: `You are an Expert Teacher with advanced expertise in discipline/faculty of {{topic}} and 
    ...
    graduate program’s timeframe and resources. Provide each idea as a concise one-line 
    description that includes the problem and a suggested solution approach, enabling the 
